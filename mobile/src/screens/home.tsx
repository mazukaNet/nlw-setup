        ...
        <View className="flex-1 bg-background px-8 pt-16 ">
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                <View className="flex-row flex-wrap">
                    ...
                    {amountOfDaysToFill > 0 && Array.from({ length: amountOfDaysToFill }).map((_, index) => <View key={index} className="bg-zinc-900 rounded-lg border-2 m-1 border=zinc-800 opacity-40" style={{ width: DAY_SIZE, height: DAY_SIZE }} />)}
                </View>
            </ScrollView>
        </View>
    );
